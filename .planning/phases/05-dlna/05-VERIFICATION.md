...
human_verification:
  - test: "Launch MyAirShow, then open BubbleUPnP (Android), VLC (desktop), or foobar2000 with UPnP plugin. Look for 'MyAirShow' in the renderer/player list."
    expected: "MyAirShow appears as a DLNA Media Renderer (DLNA-03)"
  ...
  - test: "Select MyAirShow as the renderer in a DLNA controller app, then push an MP4 or MKV video file."
    expected: "Video plays on the MyAirShow window with audio through speakers; HUD shows 'DLNA Controller' and 'DLNA' protocol (DLNA-01)"
  ...
  - test: "Select MyAirShow as the renderer, then push an MP3 or FLAC audio file."
...
**Phase Goal:** Users with DLNA controller apps can push video and audio files to MyAirShow for playback
**Verified:** 2026-03-29T01:38:43Z
...
|---|-------|--------|----------|
| 1 | A DLNA controller app can see MyAirShow listed as a Media Renderer | ? HUMAN | UpnpAdvertiser wired, SCPD XMLs written, device XML present — live network test required |
...
| `src/main.cpp` | ✓ VERIFIED | `#include "protocol/DlnaHandler.h"`, make_unique<myairshow::DlnaHandler>, setDlnaHandler, addHandler present; ordering: setDlnaHandler (line 87) before start() (line 113) confirmed |
...
|------|----|-----|--------|---------|
| `src/discovery/UpnpAdvertiser.cpp` | `src/protocol/DlnaHandler.h` | `static_cast<myairshow::DlnaHandler*>(cookie)` in upnpCallback | ✓ WIRED | Line 249: `auto* handler = static_cast<myairshow::DlnaHandler*>(cookie); return handler->handleSoapAction(event);` |
...
| `src/main.cpp` | `src/protocol/DlnaHandler.h` | `make_unique<myairshow::DlnaHandler>` + `protocolManager.addHandler` | ✓ WIRED | Lines 83, 89 confirmed |
...
| myairshow binary exists (clean build) | `ls build/linux-debug/MyAirShow` | Binary present, ninja reported no work to do | ✓ PASS |
...
|-------------|---------------|-------------|--------|----------|
| DLNA-01 | 05-02, 05-03 | User can push video files from a DLNA controller to MyAirShow for playback | ? HUMAN | Full implementation present (SetAVTransportURI → setUri, Play → playUri, uridecodebin pipeline with video branch); live controller test needed |
| DLNA-02 | 05-02, 05-03 | User can push audio files from a DLNA controller to MyAirShow for playback | ? HUMAN | SinkProtocolInfo lists 14 MIME types (audio/mpeg, audio/mp4, audio/flac, etc.); uridecodebin audio branch wired; live controller test needed |
| DLNA-03 | 05-01, 05-03 | MyAirShow appears as a DLNA Media Renderer (DMR) in DLNA controller apps | ? HUMAN | UpnpAdvertiser with MediaRenderer.xml, SCPD XMLs, and DlnaHandler SOAP routing all wired; SSDP discovery requires live network test |
...
**Test:** Launch MyAirShow, then open BubbleUPnP (Android), VLC (View > Playlist > UPnP), or foobar2000 with UPnP plugin on the same network.
**Expected:** "MyAirShow" (or the configured receiver name) appears in the renderer list as a Media Renderer.
...
**Test:** Select MyAirShow as the renderer, then push an MP4 or MKV video file from the controller. Observe the receiver window and speakers.
